...
async function resolveTarget(ctx, rawName) {
  if (!rawName) {
    ctx.reply('', ctx.embed({ color: ctx.COLORS.err, description: 'Informe o usuario alvo.' }));
    return null;
  }
  const target = await store.findMemberByName(ctx.guild.id, rawName);
  if (!target) {
  ...
  if ((await store.rank(ctx.guild.id, target.id)) >= (await store.rank(ctx.guild.id, ctx.user.id))) {
...
const commands = [
  {
    ...
    async run(ctx) {
      ...
      await store.removeMember(ctx.guild.id, target.id);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'kick', target.id, { reason });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (!target) return;
      const reason = rest.join(' ') || 'sem motivo informado';
      await store.banMember(ctx.guild.id, target.id, reason, ctx.user.id);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'ban', target.id, { reason });
      ctx.bot.deliver?.({ __memberRemoved: true, guildId: ctx.guild.id, userId: target.id });
  ...
  },
  {
    ...
    permission: 'admin',
    async run(ctx) {
      const banned = await store.listBans(ctx.guild.id);
      ...
      await store.unbanMember(ctx.guild.id, found.user.id);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'unban', found.user.id);
  ...
  },
  {
    ...
    permission: 'mod',
    async run(ctx) {
      const bans = await store.listBans(ctx.guild.id);
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      await store.logAudit(ctx.guild.id, ctx.user.id, 'mute', target.id, { reason, durationMs: ms });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      await run('UPDATE guild_members SET muted_until = 0 WHERE guild_id = ? AND user_id = ?', ctx.guild.id, target.id);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'unmute', target.id);
      ctx.bot.deliver?.({ __memberUpdated: true, guildId: ctx.guild.id, userId: target.id });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      if (!target) return;
      const reason = rest.join(' ') || 'sem motivo informado';
      await run('INSERT INTO warns (id, guild_id, user_id, moderator, reason, created_at) VALUES (?, ?, ?, ?, ?, ?)',
        newId(), ctx.guild.id, target.id, ctx.user.id, reason, Date.now());

      const total = (await get('SELECT COUNT(*) AS n FROM warns WHERE guild_id = ? AND user_id = ?', ctx.guild.id, target.id)).n;
      let extra = null;
      if (total >= 3) {
        await run('UPDATE guild_members SET muted_until = ? WHERE guild_id = ? AND user_id = ?',
      ...
      await store.logAudit(ctx.guild.id, ctx.user.id, 'warn', target.id, { reason, total });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const rows = await all('SELECT * FROM warns WHERE guild_id = ? AND user_id = ? ORDER BY created_at DESC', ctx.guild.id, target.id);
      if (!rows.length) {
      ...
      for (let i = 0; i < rows.length; i++) {
        const w = rows[i];
        const mod = await store.getUser(w.moderator);
        lines.push(`**${i + 1}.** ${w.reason} — por ${mod?.username ?? '???'} em ${new Date(w.created_at).toLocaleDateString('pt-BR')}`);
      }
      return ctx.reply('', ctx.embed({
        color: ctx.COLORS.warn,
  ...
  },
  {
    ...
    async run(ctx) {
      const target = await store.findMemberByName(ctx.guild.id, ctx.argStr);
      if (!target) return ctx.reply('Usuario nao encontrado.');
      await run('DELETE FROM warns WHERE guild_id = ? AND user_id = ?', ctx.guild.id, target.id);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'warns_cleared', target.id);
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const ids = await store.purgeMessages(ctx.channel.id, count + 1);
      for (const id of ids) ctx.bot.deliver?.({ __deleted: true, id, channelId: ctx.channel.id });
      await store.logAudit(ctx.guild.id, ctx.user.id, 'messages_purged', null, { count: ids.length, channel: ctx.channel.name });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      const target = await resolveTarget(ctx, name);
      if (!target) return;
      if (store.ROLE_RANK[role] >= (await store.rank(ctx.guild.id, ctx.user.id))) {
        return ctx.reply('', ctx.embed({ color: ctx.COLORS.err, description: 'Voce nao pode dar um cargo igual ou acima do seu.' }));
      }
      await store.setRole(ctx.guild.id, target.id, role);
      await store.logAudit(ctx.guild.id, ctx.user.id, 'role_promoted', target.id, { role });
      ctx.bot.deliver?.({ __memberUpdated: true, guildId: ctx.guild.id, userId: target.id });
      return ctx.reply('', ctx.embed({
        color: ctx.COLORS.ok,
  ...
  },
  {
    ...
    permission: 'admin',
    async run(ctx) {
      const target = await resolveTarget(ctx, ctx.argStr);
      if (!target) return;
      await store.setRole(ctx.guild.id, target.id, 'member');
      await store.logAudit(ctx.guild.id, ctx.user.id, 'role_demoted', target.id);
      ctx.bot.deliver?.({ __memberUpdated: true, guildId: ctx.guild.id, userId: target.id });
  ...
  },
  {
    ...
    async run(ctx) {
      ...
      await run('UPDATE guild_members SET nickname = ? WHERE guild_id = ? AND user_id = ?', nick, ctx.guild.id, target.id);
      ctx.bot.deliver?.({ __memberUpdated: true, guildId: ctx.guild.id, userId: target.id });
      return ctx.reply('', ctx.embed({
        color: ctx.COLORS.ok,
